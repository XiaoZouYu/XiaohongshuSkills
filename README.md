...
- **用户页信息提取**：支持抓取用户主页快照与主页笔记列表，`notes-from-profile` 会优先从 `window.__INITIAL_STATE__` 提取 `xsec_token`
...
说明：`notes-from-profile` 现在会优先从页面的 `window.__INITIAL_STATE__` 中提取主页笔记列表，因此更容易直接拿到 `note_id + xsec_token`；如果状态里拿不到，再退回 DOM 滚动抓取。
...
说明：评论巡检会默认跳过“当前商家账号已经回复过”的评论；只要该评论下已经能识别到商家账号回复，无论回复内容是什么，都会视为已回复。
