...
9. 若用户需要用户主页信息，执行 `profile-snapshot` 或 `notes-from-profile`。`notes-from-profile` 现在会优先从 `window.__INITIAL_STATE__` 抓取笔记列表与 `xsec_token`。
...
说明：`notes-from-profile` 会优先从 `window.__INITIAL_STATE__` 抓取主页笔记列表，因此商家主页场景下更容易直接拿到 `note_id + xsec_token`。
...
说明：评论巡检默认会跳过“当前商家账号已经回复过”的评论；只要评论下存在商家账号回复，无论回复内容是什么，都视为已回复。
